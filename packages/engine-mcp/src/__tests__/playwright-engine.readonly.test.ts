...
describe('PlaywrightEngine.runReadOnlyClick', () => {
  ...
  it('阻断点击处理器中的写入副作用（如 POST/PUT），放行只读 GET 请求', async () => {
    const engine = new PlaywrightEngine({ headless: true, subsystemId: 'readonly' });
    await engine.launch();
    try {
      ...
      const page = (engine as unknown as { page: {
        ...
        on: (event: string, handler: (req: any) => void) => void;
      } }).page;
      await page.route('**/source', async (route) => route.fulfill({ body: '<button id="detail" data-safe-opener aria-haspopup="dialog" onclick="fetch(\'https://readonly.example.test/api/list\').catch(e=>console.log(\'Fetch error:\',e)); Promise.resolve().then(()=>fetch(\'/toggleStatus?id=7\',{method:\'POST\'}).catch(e=>console.log(\'POST error:\',e)));">查看详情</button>' }));
      ...
      await expect(engine.runReadOnlyClick('#detail', 'action')).resolves.toMatchObject({
        status: 'blocked',
        reason: expect.stringContaining('fetch(POST)'),
      });
      expect(sideEffectWrites).toBe(0);
    } finally {
      await engine.close();
    }
  }, 15_000);

  it('阻断 WebSocket、sendBeacon、XHR 写入方法与原生 form 提交通道', async () => {
    const engine = new PlaywrightEngine({ headless: true, subsystemId: 'readonly' });
    await engine.launch();
    try {
      const page = (engine as unknown as { page: { setContent: (html: string) => Promise<void> } }).page;
      await page.setContent(`<form id="write" action="https://readonly.example.test/write"><button type="button" id="open" data-safe-opener aria-haspopup="dialog">打开详情</button></form><script>
        document.querySelector('#open').onclick=()=>{
          for (const operation of [
            ...
            ()=>{const xhr=new XMLHttpRequest();xhr.open('POST','https://readonly.example.test/toggleStatus');xhr.send()},
