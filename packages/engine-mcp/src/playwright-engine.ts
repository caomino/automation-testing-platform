...
export class PlaywrightEngine implements CaptureEngine {
  ...
  private domToModules(nodes: SemanticNode[], parentId: string | null, depth: number, subsystemId: string): ModuleNode[] {
    ...
    for (const n of nodes) {
      const id = randomUUID();
      // 边界（2026-09-20 用户裁定）：第一次探索只到「菜单子目录/功能页」粒度，
      // 不产出 type:'action' 的动作级功能点（动作级探索归用例阶段）。
      // 降级 DOM 路径原先把可交互节点标成 action → 会污染功能点表；此处改为 page 并保持 needs_review 语义，
      // 由 markTreeNeedsReview 统一标注「降级来源，需人工确认」。
      const type: ModuleNode['type'] = n.children.length ? 'module' : 'page';
