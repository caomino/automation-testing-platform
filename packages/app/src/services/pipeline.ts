...
export function moduleTreeToFeatureTable(tree: ModuleNodeView[], systemName: string): FeatureRowView[] {
  ...
  const walk = (nodes: ModuleNodeView[], path: string[]): void => {
    for (const n of nodes) {
      if (n.type === 'action') {
        index += 1;
        const mainLabel = path[0] || '';
        const subLabel = path[1] || '';
        const featureLabel = path[2] || '';
        
        const mainAbbr = toAbbrTokenWithLabel('', mainLabel);
        const subAbbr = toAbbrTokenWithLabel('', subLabel || 'X');
      ...
      } else if (n.type === 'module' || n.type === 'page') {
