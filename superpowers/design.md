...
| **S5** | **陕西人大统一平台**（**门户 + 配置的多个子系统**，创建项目时选父门户+浏览器捕获 URL） | **admin/<REDACTED>** | **政府系统（待浏览器确认）** | **⭐ 完全适配（验收放最后，设计提前兼容）** | **只读结构 + 菜单可操作 + 数据不可操作** |

> S5 是验收硬指标：`https://<REDACTED>/typtnew/`（真实地址存于加密存储 infra-cred），只读探索、点菜单导航，禁止增删改数据。密码仅存加密存储（infra-cred）。

**S5 实测结构（2026-08-15 浏览器探测确认）**：
- 技术栈：Vue + Element UI + hash 路由，**无 iframe、无验证码**（admin/<REDACTED> 直接登录）。
